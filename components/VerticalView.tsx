...
type Props =
  | Pick<ScrollViewProps, 'children' | 'style'>
  | Pick<RefreshControlProps, 'onRefresh'>;
const VerticalView = React.forwardRef<ScrollView, Props>(
  ({ children, style, onRefresh = undefined }: any, ref) => {
    const { colors } = useTheme();
    const [refreshing] = React.useState(false); // todo: setRefreshing
    return (
      <ScrollView
        style={[
          { display: 'flex', flex: 1, backgroundColor: colors.background },
        ]}
