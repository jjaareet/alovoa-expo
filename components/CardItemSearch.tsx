...
const CardItem = ({
  ...
  function calcImageSize(): number {
    ...
    return width + 300 < height
      ? width - cardPadding * 2
      : height / (height < 900 ? 2 : 1.5) - cardPadding * 2;
  }
  ...
  async function load() {
    let hideThresholdString = await Global.GetStorage(
      Global.STORAGE_ADV_SEARCH_HIDE_THRESHOLD,
    );
    let hideThreshold = hideThresholdString
      ? Number(hideThresholdString)
      : Global.DEFAULT_HIDE_THRESHOLD;
  ...
  return (
    <View
      style={[
        styles.containerCardItem,
        {
          ...
          height:
            height - NAVIGATION_BAR_HEIGHT - STATUS_BAR_HEIGHT - cardPadding,
    ...
    >
      {tooManyReports && (
        ...
        >
          <View style={{ gap: 24 }}>
            <Text>
              {Global.format(
                i18n.t('profile.search.report-card'),
                user.firstName,
              )}
            </Text>
            <View style={{ flexDirection: 'row' }}>
              <Button
                style={{ flex: 1 }}
                onPress={() => setTooManyReports(false)}
              >
                {i18n.t('profile.search.report-card-show')}
              </Button>
              <Button
                mode="contained"
                ...
                onPress={() => onHideUser()}
              >
                {i18n.t('profile.search.report-card-dismiss')}
              </Button>
            </View>
          </View>
        </View>
      ...
      {!tooManyReports && (
        <Pressable
          style={{ flexGrow: 1 }}
          onPress={() => Global.nagivateProfile(user)}
        >
          {/* IMAGE */}
          ...
          >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <MaterialCommunityIcons
                name="map-marker"
                size={18}
                style={[{ paddingRight: 4, color: colors?.secondary }]}
              />
            ...
            <View style={styles.marginBottom4}>
              <View style={{ alignSelf: 'stretch', paddingBottom: 8 }}>
                <Text style={styles.marginBottom4}>
                  {i18n.t('profile.interests-common')}
                </Text>
                <View
                  style={{
                    display: 'flex',
      ...
      {!tooManyReports && (
        <View style={styles.actionsCardItem}>
          <TouchableOpacity
            style={[styles.button, { backgroundColor: GRAY, marginRight: 24 }]}
            onPress={() => onHideUser()}
          >
            <Icon
              name="close"
