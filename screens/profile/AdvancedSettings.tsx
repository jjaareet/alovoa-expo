...
const AdvancedSettings = ({ route }: Props) => {
  ...
  const [hideThreshold, setHideThreshold] = React.useState(
    defaultHideThresholdString,
  );

  async function load() {
    setLatitude(String(user.locationLatitude));
    setLongitude(String(user.locationLongitude));
    let timeoutStorage = await Global.GetStorage(
      Global.STORAGE_ADV_SEARCH_GPSTIMEOPUT,
    );
    setGpsTimeout(timeoutStorage ? timeoutStorage : defaultTimeoutString);

    let hideThresholdStorage = await Global.GetStorage(
      Global.STORAGE_ADV_SEARCH_HIDE_THRESHOLD,
    );
    setHideThreshold(
      hideThresholdStorage ? hideThresholdStorage : defaultHideThresholdString,
    );
  }
  ...
  async function uploadLocation() {
    if (!isNaN(Number(latitude)) && !isNaN(Number(longitude))) {
      Global.Fetch(
        Global.format(URL.USER_UPDATE_LOCATION, latitude, longitude),
        'post',
      );
    }
  }
