...
const Onboarding = ({ route: _r, navigation: _n }: Props) => {
  ...
  const [isGenderFemaleEnabled, setIsGenderFemaleEnabled] =
  ...
  async function pickImage() {
    ...
    if (imageData) {
      setImage(
        imageData.startsWith('file:') ? imageData : IMAGE_HEADER + imageData,
      );
  ...
  const toggleNotificationLikeSwitch = () =>
    setIsNotificationLike(previousState => !previousState);
  const toggleNotificationChatSwitch = () =>
    setIsNotificationChat(previousState => !previousState);
  ...
  async function submit() {
    if (!image) {
      ...
      !isGenderMaleEnabled &&
      !isGenderFemaleEnabled &&
      !isGenderOtherEnabled
    ) {
      ...
      >
        <View style={[styles.view]}>
          ...
          <Text style={styles.title}>
            {i18n.t('profile.onboarding.profile-picture')}
          </Text>
          ...
          <Text style={styles.warning}>
            {i18n.t('profile.onboarding.profile-picture-subtitle')}
          </Text>
        </View>
        <KeyboardAvoidingView
        ...
        >
          ...
          />
          <Text style={styles.title}>
            {i18n.t('profile.onboarding.description')}
          </Text>
        ...
        <View style={[styles.view]}>
          ...
          <View>
            <View style={{ flexDirection: 'row' }}>
              <Checkbox.Item
                onPress={toggleGenderMaleSwitch}
                position="leading"
                status={isGenderMaleEnabled ? 'checked' : 'unchecked'}
                label={i18n.t('gender.male')}
              />
            </View>
            <View style={{ flexDirection: 'row' }}>
              <Checkbox.Item
                onPress={toggleGenderFemaleSwitch}
                position="leading"
                status={isGenderFemaleEnabled ? 'checked' : 'unchecked'}
                label={i18n.t('gender.female')}
              />
            </View>
            <View style={{ flexDirection: 'row' }}>
              <Checkbox.Item
                onPress={toggleGenderOtherSwitch}
                position="leading"
                status={isGenderOtherEnabled ? 'checked' : 'unchecked'}
        ...
        <View style={[styles.view]}>
          ...
          >
            <RadioButton.Item
              label={i18n.t('profile.intention.meet')}
              value={String(IntentionEnum.MEET)}
              style={{ flexDirection: 'row-reverse' }}
            />
            <RadioButton.Item
              label={i18n.t('profile.intention.date')}
              value={String(IntentionEnum.DATE)}
              style={{ flexDirection: 'row-reverse' }}
            />
            <RadioButton.Item
              label={i18n.t('profile.intention.sex')}
              value={String(IntentionEnum.SEX)}
              disabled={!isLegal}
              style={{ flexDirection: 'row-reverse' }}
            />
          </RadioButton.Group>
          ...
          <Text style={styles.warning}>
            {i18n.t('profile.intention.warning')}
          </Text>
        </View>
        <View style={[styles.view]}>
          <SvgInterests
          ...
          <InterestView
            data={interests}
            setInterestsExternal={setInterests}
          ></InterestView>
        ...
        <View style={[styles.view]}>
          ...
          />
          <Text style={styles.title}>
            {i18n.t('profile.settings.notification')}
          </Text>
          <View>
            <View style={{ flexDirection: 'row' }}>
              <Checkbox.Item
                onPress={toggleNotificationLikeSwitch}
                position="leading"
                status={isNotificationLike ? 'checked' : 'unchecked'}
                label={i18n.t('profile.settings.email.like')}
              />
            </View>
            <View style={{ flexDirection: 'row' }}>
              <Checkbox.Item
                onPress={toggleNotificationChatSwitch}
                position="leading"
                status={isNotificationChat ? 'checked' : 'unchecked'}
                label={i18n.t('profile.settings.email.chat')}
              />
            </View>
          </View>
        </View>
        <View style={[styles.view]}>
          ...
          />
          <Text style={styles.title}>
            {i18n.t('profile.onboarding.match.title')}
          </Text>
          <Button
            mode="contained"
            icon="heart"
          ...
          >
            <Text style={{ color: 'white' }}>
              {i18n.t('profile.onboarding.submit')}
            </Text>
          </Button>
          <Text style={styles.warning}>
            {i18n.t('profile.onboarding.match.subtitle')}
          </Text>
        </View>
      </SwiperFlatList>
      <FAB
